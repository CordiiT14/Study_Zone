...
const Sessions = ({sessionToggle, workTime, setWorkTime, breakTime, setBreakTime}) => {
    ...
    return(
        <div id='session-toggle'>
                <div className="work" onClick={sessionToggle}>
                    <label>Work</label>
                    <input type="number" onChange={setNewWorkTime} placeholder={workTimeInMinutes} min={0} max={60}></input>
                </div>
                <div className="break" onClick={sessionToggle}>
                    <label>Break</label>
                    <input type="number" onChange={setNewBreakTime} placeholder={breakTimeInMinutes} min={0} max={60}></input>
                </div>
            </div>
    )
}
